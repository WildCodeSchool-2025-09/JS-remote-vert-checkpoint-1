...
function findSeniors(persons) {
  const seniorDev = persons.filters(persons => persons.job === 'web dev' && persons.experience > =5);
  const seniorDataAnalyst = persons.filters(persons => persons.job === 'data analyst' && persons.experience >= 5);
  return [seniorDev, seniorDataAnalyst] 
}
